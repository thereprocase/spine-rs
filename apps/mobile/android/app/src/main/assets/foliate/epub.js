...
${doc.querySelector('parsererror').innerText}`)
    ...
    async init() {
        ...
        this.sections = this.resources.spine.map((spineItem, index) => {
            ...
            // SPINE-PATCH: lazy size. Upstream eagerly evaluates
            // `this.getSize(item.href)` for every spine item during
            // EPUB.init(); on a 3000-chapter fanfic compendium that
            // fires 3000 concurrent HEAD requests through
            // EpubResourceHandler before first paint, swamping the
            // resource handler and stretching cold-open to 8-15s.
            // The lazy getter defers evaluation until something
            // actually reads `.size` (currently only SectionProgress
            // when the user moves through the book), keeping the
            // Promise<number> contract intact for consumers.
            // Re-applied by scripts/vendor-foliate.mjs's
            // applySpinePatches() after each upstream fetch.
            // (Legolas review N1-N6 critical #5.)
            const epubSelf = this
            let _sizeCache
            return {
                id: item.href,
                ...
                get size() { return _sizeCache ?? (_sizeCache = epubSelf.getSize(item.href)) },
