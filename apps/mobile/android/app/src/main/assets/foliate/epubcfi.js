...
// SPINE-PATCH: tolerate undefined (e.g. nodes[0] when getChildNodes
// returned [] because doc.body was still attaching). Original used
// destructuring which crashes on undefined.
const isTextNode = node => node != null && (node.nodeType === 3 || node.nodeType === 4)
const isElementNode = node => node != null && node.nodeType === 1

const getChildNodes = (node, filter) => {
    // SPINE-PATCH: getCFI fires from spine-host's tap handler the moment a
    // click lands. If the iframe doc body is still attaching the node passed
    // in can be null — return [] so the CFI is just empty rather than throw.
